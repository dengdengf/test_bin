...
def model_load(path, device, warn_on_old_format=True):
    ...
    elif saved['model_name'] == 'MultiModalContrastiveModel':
        from .multimodal_model import MultiModalContrastiveModel
        model = MultiModalContrastiveModel(**saved['params'])
    else:
        raise ValueError(f"Unknown model type: {saved['model_name']}")
