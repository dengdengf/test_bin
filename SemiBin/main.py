...
def parse_args(args):
    ...
    for p in [single_easy_bin, multi_easy_bin, generate_sequence_features_single]:
        ...
        default_dnabert_path = os.path.join(base_dir, 'DNABERT-S')
        p.add_argument('--dnabert-model',
                       required=False,
                       type=str,
                       help='Path to the pre-trained DNABERT model directory (containing config.json and pytorch_model.bin).',
                       dest='dnabert_model_path',
                       default=default_dnabert_path,
                       )
        p.add_argument('--dnabert-python',
                       required=False,
                       type=str,
                       help='Python executable used for DNABERT feature extraction. If not set, use $SEMIBIN_DNABERT_PYTHON or the current Python interpreter.',
                       dest='dnabert_python',
                       default=None)
...
def training(logger, contig_fasta,
    ...
    from .multimodal_model import train_multimodal_self, load_multimodal_embeddings
    import pandas as pd
    ...
    if args.training_type == 'semi':
        ...
        if multimodal_enabled:
            try:
                dnabert_embeddings = []
                dnabert_split_embeddings = []
                for datapath, splitpath in zip(data, data_split):
                    whole_embedding, split_embedding = load_multimodal_embeddings(
                        logger, datapath, splitpath)
                    dnabert_embeddings.append(whole_embedding)
                    dnabert_split_embeddings.append(split_embedding)
                logger.info('Using multimodal short-read training.')
                model = train_multimodal_self(
                    logger,
                    data,
                    data_split,
                    dnabert_embeddings,
                ...
                logger.warning(f'Falling back to legacy self-supervised training because multimodal setup failed: {exc}')
...
def binning_short(logger, data, minfasta,
    ...
    # 2. [新增] 加载 DNABERT Embedding
    dnabert_embedding = None
    dnabert_npy_path = os.path.join(output, 'dnabert_embedding.npy')
    dnabert_name_path = os.path.join(output, 'dnabert_contig_names.txt')

    if os.path.exists(dnabert_npy_path):
        try:
            logger.info(f"Found DNABERT embedding file: {dnabert_npy_path}")
            raw_embedding = np.load(dnabert_npy_path).astype(np.float32)
            if os.path.exists(dnabert_name_path):
                with open(dnabert_name_path, 'r', encoding='utf-8') as handle:
                    feature_names = [line.rstrip('\n') for line in handle]
                if feature_names != data.index.astype(str).tolist():
                    raise ValueError('DNABERT feature order does not match data.csv order')

            if len(raw_embedding) != len(data):
                logger.warning(f"DNABERT embedding length ({len(raw_embedding)}) != data length ({len(data)}). Skipping DNABERT.")
            else:
                if raw_embedding.shape[1] > 128:
                    logger.info(f"Reducing DNABERT dim from {raw_embedding.shape[1]} to 128 via PCA...")
                    pca = PCA(n_components=128, random_state=args.random_seed if args.random_seed else 42)
                    dnabert_embedding = pca.fit_transform(raw_embedding)
                else:
                    dnabert_embedding = raw_embedding
                
                logger.info("DNABERT embedding loaded successfully.")
        except Exception as e:
            logger.error(f"Failed to load DNABERT embedding: {e}")
            dnabert_embedding = None
    else:
        logger.info("No DNABERT embedding found. Running standard SemiBin2 pipeline.")

    if getattr(model, 'is_multimodal', False) and dnabert_embedding is None:
        raise ValueError('The saved model is multimodal, but dnabert_embedding.npy is missing or invalid.')

    # 3. 调用 Cluster，传入 dnabert_embedding
    cluster(
        logger,
        model=model,
        data=data,
        ...
        binned_length=binned_length,
        minfasta=minfasta,
        dnabert_embedding=dnabert_embedding # <--- 新增传参
    )
...
def run_dnabert_extraction(logger, output_dir, contig_fasta, dnabert_model_path, dnabert_python_exe=None):
    """
    Extract DNABERT features for whole contigs and split contigs used in short-read training.
    """
    data_csv_path = os.path.join(output_dir, 'data.csv')
    data_split_path = os.path.join(output_dir, 'data_split.csv')
    output_npy_path = os.path.join(output_dir, 'dnabert_embedding.npy')
    output_names_path = os.path.join(output_dir, 'dnabert_contig_names.txt')
    split_output_npy_path = os.path.join(output_dir, 'dnabert_split_embedding.npy')
    split_output_names_path = os.path.join(output_dir, 'dnabert_split_contig_names.txt')

    if not os.path.exists(data_csv_path):
        logger.error(f'data.csv not found at {data_csv_path}')
        sys.exit(1)

    if not os.path.exists(data_split_path):
        logger.info('data_split.csv was not generated for this run. Skip DNABERT extraction.')
        return

    python_candidates = [
        dnabert_python_exe,
        os.environ.get('SEMIBIN_DNABERT_PYTHON'),
        sys.executable,
    ]
    dnabert_python_exe = next((candidate for candidate in python_candidates if candidate and os.path.exists(candidate)), None)
    if dnabert_python_exe is None:
        logger.error('Cannot find a usable Python executable for DNABERT extraction. Set --dnabert-python or $SEMIBIN_DNABERT_PYTHON.')
        sys.exit(1)

    logger.info('Preparing DNABERT extraction...')

    temp_script_path = os.path.join(output_dir, 'run_dnabert_temp.py')
    script_content = f"""
import os
import sys
...
DATA_PATH = r"{data_csv_path}"
DATA_SPLIT_PATH = r"{data_split_path}"
WHOLE_OUTPUT = r"{output_npy_path}"
WHOLE_NAMES = r"{output_names_path}"
SPLIT_OUTPUT = r"{split_output_npy_path}"
...
def main():
    whole_df = pd.read_csv(DATA_PATH, index_col=0)
    split_df = pd.read_csv(DATA_SPLIT_PATH, index_col=0)
    ...
    model = AutoModel.from_pretrained(MODEL_PATH, trust_remote_code=True, local_files_only=True).to(device)
    model.eval()
    ...
    print('[DNABERT] Feature extraction finished.')

if __name__ == '__main__':
    main()
"""

    with open(temp_script_path, 'w', encoding='utf-8') as handle:
        handle.write(script_content)

    env = os.environ.copy()
    dnabert_lib_path = os.path.join(os.path.dirname(dnabert_python_exe), '../lib')
    env['LD_LIBRARY_PATH'] = dnabert_lib_path + ':' + env.get('LD_LIBRARY_PATH', '')
    cmd = [dnabert_python_exe, temp_script_path]

    logger.info(f'Executing DNABERT script with: {dnabert_python_exe}')
    try:
        subprocess.run(cmd, check=True, env=env)
        logger.info('DNABERT extraction completed.')
    except subprocess.CalledProcessError as exc:
        logger.error(f'DNABERT extraction failed. Exit code: {exc.returncode}')
        sys.exit(1)
...
# ----------------------------------------------------------------------
    # !!! [插入点] 调用独立函数生成 DNABERT Embedding !!!
    # ----------------------------------------------------------------------
    # 检查用户是否提供了模型路径，如果有，则调用跨环境提取函数

    if getattr(args, 'depth_metabat2', None):
        logger.info('Skipping DNABERT extraction because --depth-metabat2 does not generate data_split.csv.')
    elif hasattr(args, 'dnabert_model_path') and args.dnabert_model_path is not None:
        run_dnabert_extraction(
            logger=logger, 
            output_dir=args.output, 
            contig_fasta=args.contig_fasta, 
            dnabert_model_path=args.dnabert_model_path,
            dnabert_python_exe=getattr(args, 'dnabert_python', None),
        )
    # ----------------------------------------------------------------------
...
def multi_easy_binning(logger, args, device):
    ...
    for sample_index, sample in enumerate(sample_list):
        ...
        if getattr(args, 'dnabert_model_path', None) is not None:
            run_dnabert_extraction(
                logger=logger,
                output_dir=os.path.join(args.output, 'samples', sample),
                contig_fasta=sample_fasta,
                dnabert_model_path=args.dnabert_model_path,
                dnabert_python_exe=getattr(args, 'dnabert_python', None),
            )
        if args.training_type == 'semi':
