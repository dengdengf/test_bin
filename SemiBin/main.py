...
def parse_args(args):
    ...
    for p in [single_easy_bin, generate_sequence_features_single]:
        ...
        p.add_argument('--dnabert-model',
                       ...
                       dest='dnabert_model_path', # 在代码中将通过 args.dnabert_model_path 访问
...
def run_dnabert_extraction(logger, output_dir, contig_fasta, dnabert_model_path):
    """
    独立函数：使用 dnaberts 环境的绝对路径 python 提取特征
    """
    ...
    logger.info(f"Preparing DNABERT extraction...")

    # ------------------------------------------------------------------
    # 1. 关键修改：指定 dnaberts 环境的 Python 解释器绝对路径
    # 请根据你的实际 conda 安装路径修改下面这一行！
    # 通常在 /home/用户名/anaconda3/envs/环境名/bin/python
    # 或者 /home/用户名/miniconda3/envs/环境名/bin/python
    # ------------------------------------------------------------------
    dnabert_python_exe = "/home/wangjingyuan/anaconda3/envs/dnaberts/bin/python"
    
    
    # 检查解释器是否存在
    if not os.path.exists(dnabert_python_exe):
        logger.error(f"Cannot find dnaberts python at: {dnabert_python_exe}")
        logger.error("Please update the 'dnabert_python_exe' variable in the script.")
        sys.exit(1)
...
def main():
    ...
    data_csv = r"{data_csv_path}"
    output_path = r"{output_npy_path}"
    
    print(f"[DNABERT] Loading data.csv...")
    try:
        df = pd.read_csv(data_csv, index_col=0)
        target_contigs = set(df.index.astype(str))
        target_list_ordered = df.index.astype(str).tolist()
        
        print(f"[DNABERT] Loading model from {{model_path}}...")
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        tokenizer = AutoTokenizer.from_pretrained(model_path, trust_remote_code=True, local_files_only=True)
        model = AutoModel.from_pretrained(model_path, trust_remote_code=True).to(device)
        model.eval()

        print(f"[DNABERT] Reading sequences...")
        seq_dict = {{}}
        for record in SeqIO.parse(fasta_path, "fasta"):
            if str(record.id) in target_contigs:
                seq_dict[str(record.id)] = str(record.seq)

        ordered_seqs = []
        for name in target_list_ordered:
            if name in seq_dict:
                ordered_seqs.append(seq_dict[name])
            else:
                print(f"Error: Contig {{name}} missing in FASTA.")
                sys.exit(1)

        feature_list = []
        print(f"[DNABERT] Extracting features for {{len(ordered_seqs)}} contigs...")
        
        for seq in tqdm(ordered_seqs):
            inputs = tokenizer(seq, return_tensors='pt', padding="longest", max_length=5000, truncation=True)["input_ids"]
        ...
        print(f"[DNABERT] Success! Saved to {{output_path}}")

    except Exception as e:
        print(f"[DNABERT] Critical Error: {{e}}")
        sys.exit(1)
...
"""
    ...
    try:
        ...
        logger.error(f"DNABERT extraction failed. Exit code: {e.returncode}")
        sys.exit(1)
    finally:
        # 调试通过后可以取消注释来删除临时文件
...
# ----------------------------------------------------------------------
    ...
    if hasattr(args, 'dnabert_model_path') and args.dnabert_model_path is not None:
